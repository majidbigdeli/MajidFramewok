...
{
    public class MajidUserManager<TRole, TUser> : UserManager<TUser>, IDomainService
        ...
        where TUser : MajidUser<TUser>
    {
        protected IUserPermissionStore<TUser> UserPermissionStore
        {
            get
            {
                ...
                {
                    throw new MajidException("Store is not IUserPermissionStore");
                }
        ...
        protected MajidUserStore<TRole, TUser> MajidUserStore { get; }
        ...
        /// <exception cref="MajidException">Throws exception if no user found with given id</exception>
        public virtual async Task<TUser> GetUserByIdAsync(long userId)
        {
            var user = await FindByIdAsync(userId.ToString());
            if (user == null)
            {
                throw new MajidException("There is no user with id: " + userId);
            }
        ...
        public override async Task<IdentityResult> UpdateAsync(TUser user)
        {
            ...
            if (user.UserName != MajidUserBase.AdminUserName)
            {
                if ((await GetOldUserNameAsync(user.Id)) == MajidUserBase.AdminUserName)
                {
                    throw new UserFriendlyException(string.Format(L("CanNotRenameAdminUser"), MajidUserBase.AdminUserName));
                }
            }
        ...
        public override async Task<IdentityResult> DeleteAsync(TUser user)
        {
            if (user.UserName == MajidUserBase.AdminUserName)
            {
                throw new UserFriendlyException(string.Format(L("CanNotDeleteAdminUser"), MajidUserBase.AdminUserName));
            }
        ...
        {
            await MajidUserStore.UserRepository.EnsureCollectionLoadedAsync(user, u => u.Roles);
        ...
        private async Task CheckMaxUserOrganizationUnitMembershipCountAsync(int? tenantId, int requestedCount)
        {
            ...
            {
                throw new MajidException($"Can not set more than {maxCount} organization unit for a user!");
            }
        }
        ...
        public virtual async Task InitializeOptionsAsync(int? tenantId)
        {
            ...
            //Lockout
            Options.Lockout.AllowedForNewUsers = await IsTrueAsync(MajidZeroSettingNames.UserManagement.UserLockOut.IsEnabled, tenantId);
            Options.Lockout.DefaultLockoutTimeSpan = TimeSpan.FromSeconds(await GetSettingValueAsync<int>(MajidZeroSettingNames.UserManagement.UserLockOut.DefaultAccountLockoutSeconds, tenantId));
            Options.Lockout.MaxFailedAccessAttempts = await GetSettingValueAsync<int>(MajidZeroSettingNames.UserManagement.UserLockOut.MaxFailedAccessAttemptsBeforeLockout, tenantId);

            //Password complexity
            Options.Password.RequireDigit = await GetSettingValueAsync<bool>(MajidZeroSettingNames.UserManagement.PasswordComplexity.RequireDigit, tenantId);
            Options.Password.RequireLowercase = await GetSettingValueAsync<bool>(MajidZeroSettingNames.UserManagement.PasswordComplexity.RequireLowercase, tenantId);
            Options.Password.RequireNonAlphanumeric = await GetSettingValueAsync<bool>(MajidZeroSettingNames.UserManagement.PasswordComplexity.RequireNonAlphanumeric, tenantId);
            Options.Password.RequireUppercase = await GetSettingValueAsync<bool>(MajidZeroSettingNames.UserManagement.PasswordComplexity.RequireUppercase, tenantId);
            Options.Password.RequiredLength = await GetSettingValueAsync<int>(MajidZeroSettingNames.UserManagement.PasswordComplexity.RequiredLength, tenantId);
        }
        ...
        public override async Task<IList<string>> GetValidTwoFactorProvidersAsync(TUser user)
        {
            ...
            {
                if (provider == "Email" &&
                    !await IsTrueAsync(MajidZeroSettingNames.UserManagement.TwoFactorLogin.IsEmailProviderEnabled, user.TenantId))
                {
                    continue;
                }

                if (provider == "Phone" &&
                    !await IsTrueAsync(MajidZeroSettingNames.UserManagement.TwoFactorLogin.IsSmsProviderEnabled, user.TenantId))
                {
                    continue;
                }
        ...
        private MultiTenancySides GetCurrentMultiTenancySide()
        {
            ...
            return MajidSession.MultiTenancySide;
        }
