...
{
    public class MajidRoleManager<TRole, TUser> : RoleManager<TRole>, IDomainService
        ...
        where TUser : MajidUser<TUser>
    {
        ...
        private IRolePermissionStore<TRole> RolePermissionStore
        {
            get
            {
                ...
                {
                    throw new MajidException("Store is not IRolePermissionStore");
                }
        ...
        protected MajidRoleStore<TRole, TUser> MajidStore { get; }
        ...
        /// <exception cref="MajidException">Throws exception if no role with given id</exception>
        public virtual async Task<TRole> GetRoleByIdAsync(int roleId)
        {
            var role = await FindByIdAsync(roleId.ToString());
            if (role == null)
            {
                throw new MajidException("There is no role with id: " + roleId);
            }
        ...
        /// <param name="roleName">Role name</param>
        /// <returns>Role</returns>
        /// <exception cref="MajidException">Throws exception if no role with given roleName</exception>
        public virtual async Task<TRole> GetRoleByNameAsync(string roleName)
        {
            var role = await FindByNameAsync(roleName);
            if (role == null)
            {
                throw new MajidException("There is no role with name: " + roleName);
            }
        ...
        private async Task<RolePermissionCacheItem> GetRolePermissionCacheItemAsync(int roleId)
        {
            ...
            return await _cacheManager.GetRolePermissionCache().GetAsync(cacheKey, async () =>
            {
                ...
                if (role == null)
                {
                    throw new MajidException("There is no role with given id: " + roleId);
                }
