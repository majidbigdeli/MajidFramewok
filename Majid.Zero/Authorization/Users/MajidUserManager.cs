...
namespace Majid.Authorization.Users
{
    ...
    /// </summary>
    public abstract class MajidUserManager<TRole, TUser>
        ...
        where TUser : MajidUser<TUser>
    {
        ...
        public MajidUserStore<TRole, TUser> MajidStore { get; }
        ...
        public async override Task<ClaimsIdentity> CreateIdentityAsync(TUser user, string authenticationType)
        {
            ...
            {
                identity.AddClaim(new Claim(MajidClaimTypes.TenantId, user.TenantId.Value.ToString(CultureInfo.InvariantCulture)));
            }
        ...
        private async Task CheckMaxUserOrganizationUnitMembershipCountAsync(int? tenantId, int requestedCount)
        {
            ...
            {
                throw new MajidException(string.Format("Can not set more than {0} organization unit for a user!", maxCount));
            }
        }
        ...
        public virtual Task<List<TUser>> GetUsersInOrganizationUnit(OrganizationUnit organizationUnit, bool includeChildren = false)
        {
            ...
            {
                var query = from uou in _userOrganizationUnitRepository.GetAll()
                            join user in MajidStore.Users on uou.UserId equals user.Id
        ...
        public virtual void RegisterTwoFactorProviders(int? tenantId)
        {
            ...
            if (!IsTrue(MajidZeroSettingNames.UserManagement.TwoFactorLogin.IsEnabled, tenantId))
            {
                return;
            }

            if (EmailService != null &&
                IsTrue(MajidZeroSettingNames.UserManagement.TwoFactorLogin.IsEmailProviderEnabled, tenantId))
            {
            ...
            if (SmsService != null &&
                IsTrue(MajidZeroSettingNames.UserManagement.TwoFactorLogin.IsSmsProviderEnabled, tenantId))
            {
                RegisterTwoFactorProvider(
                    L("Sms"),
        ...
        {
            UserLockoutEnabledByDefault = IsTrue(MajidZeroSettingNames.UserManagement.UserLockOut.IsEnabled, tenantId);
            DefaultAccountLockoutTimeSpan = TimeSpan.FromSeconds(GetSettingValue<int>(MajidZeroSettingNames.UserManagement.UserLockOut.DefaultAccountLockoutSeconds, tenantId));
            MaxFailedAccessAttemptsBeforeLockout = GetSettingValue<int>(MajidZeroSettingNames.UserManagement.UserLockOut.MaxFailedAccessAttemptsBeforeLockout, tenantId);
        }
