...
using Majid.Xml.Extensions;

namespace Majid.Localization.Dictionaries.Xml
{
    /// <summary>
    ...
    public class XmlLocalizationDictionary : LocalizationDictionary
    {
        ...
        public static XmlLocalizationDictionary BuildFomFile(string filePath)
        {
            ...
            {
                throw new MajidException("Invalid localization file format! " + filePath, ex);
            }
        }
        ...
        public static XmlLocalizationDictionary BuildFomXmlString(string xmlString)
        {
            ...
            {
                throw new MajidException("A Localization Xml must include localizationDictionary as root node.");
            }

            var cultureName = localizationDictionaryNode[0].GetAttributeValueOrNull("culture");
            if (string.IsNullOrEmpty(cultureName))
            {
                throw new MajidException("culture is not defined in language XML file!");
            }
            ...
            if (textNodes != null)
            {
                foreach (XmlNode node in textNodes)
                {
                    ...
                    {
                        throw new MajidException("name attribute of a text is empty in given xml string.");
                    }
            ...
            {
                throw new MajidException("A dictionary can not contain same key twice. There are some duplicated names: " + dublicateNames.JoinAsString(", "));
            }
